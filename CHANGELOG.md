...
- **Brought 4 sibling `mcp-adapter/*` abilities to the same `show_in_rest` + `mcp.public` meta-shape as `GetAbilityInfoAbility` (Issue [#137](https://github.com/Wicked-Evolutions/abilities-mcp-adapter/issues/137)).** PR #133 / #135 corrected the meta-block placement of `'show_in_rest' => true` for `GetAbilityInfoAbility` only; the four sibling abilities under the `mcp-adapter/*` namespace (`GetStartedAbility`, `DiscoverAbilitiesAbility`, `BatchExecuteAbility`, `ExecuteAbilityAbility`) were never touched and continued to fail the default MCP server's exposure gate with `-32008 Permission denied: not exposed via MCP`. `GetStartedAbility` and `DiscoverAbilitiesAbility` had `meta.mcp.public => true` but were missing `meta.show_in_rest => true`; `BatchExecuteAbility` and `ExecuteAbilityAbility` were missing both. Meta-block only — no logic, schema, callback, or permission semantics changed (Principle 10). Closes [#137](https://github.com/Wicked-Evolutions/abilities-mcp-adapter/issues/137).

- **Default MCP server ID renamed from `mcp-adapter-default-server` to `abilities-mcp-adapter-default-server` to avoid collision with the official `wordpress/mcp-adapter` library (Issue [#134](https://github.com/Wicked-Evolutions/abilities-mcp-adapter/issues/134)).** The official reference library — vendored by FluentKit's `fluent-toolkit` v2.0.3 (via `libs/mcp-adapter/`) and installable as a standalone plugin — registers a server with the same legacy ID, and WordPress's server registry rejects duplicates first-creator-wins. On dev2.helenawillow.com (2026-05-21) after the #131 timing fix, FluentKit's bundle won the registration race and our `DefaultServerFactory::create` failed with `duplicate_server_id`. Renaming our server gives us a distinct namespace that aligns with our product slug and lets us co-exist with the official adapter rather than displacing it. Touch points: 7 occurrences across 5 adapter source files (`src/Servers/DefaultServerFactory.php`, `src/Auth/OAuth/DiscoveryEndpoints.php`, `src/Auth/OAuth/McpResourcePath.php`, `src/Auth/OAuth/helpers.php`, `src/RateLimit/BurstHarness.php`) plus the operator-facing CLI tool `bin/rate-limit-burst.php` and all test fixtures referencing the OAuth resource URL. Our default server now lives at `/wp-json/mcp/abilities-mcp-adapter-default-server`. **Operator-impact window:** the paired bridge release (`abilities-mcp` v1.6.5, shipping immediately after v1.4.9) updates the bridge's URL path to match. Between the two releases: sites running FluentKit's bundle continue to reach FluentKit's server at the old URL (no breakage); sites without any other adapter need to upgrade the bridge to v1.6.5 (or run the bridge's migration helper) before AI clients can reach our server again. Bridge follow-up tracked in `abilities-mcp` repo. Closes [#134](https://github.com/Wicked-Evolutions/abilities-mcp-adapter/issues/134).

- **Ability errors no longer conflate not-found, not-exposed, and scope-denied under one code; added recovery hint (Issue [#140](https://github.com/Wicked-Evolutions/abilities-mcp-adapter/issues/140)).** A wrong ability name passed to `mcp-adapter/execute-ability` (or `get-ability-info`) rendered as `-32008 Permission denied: Ability 'X' not found` — a name-resolution failure disguised as an auth failure, which misled AI clients toward credential/scope debugging and was the upstream cause of the "dev2 has no discovery" operator incident. The bug was a render bypass: `ToolsHandler` sent permission-callback `WP_Error`s through `McpErrorFactory::permission_denied()` directly, so a permission-phase `ability_not_found` never reached `McpErrorMapper` (which already maps `ability_not_found → -32003`). The execution path at `ToolsHandler` already used the mapper correctly; the permission path now does too (routed via `McpErrorMapper::from_wp_error` with a `PERMISSION_DENIED` default, so genuine permission/auth errors keep `-32008`). The three conditions are now distinguishable by code + a `data.reason` field: not-found → `-32003`, `data.reason: "not_found"` (plus `data.hint` pointing at `mcp-adapter/discover-abilities`); not-exposed-via-MCP → `-32008`, `data.reason: "not_exposed"`; scope-denied → unchanged structured `required_scope` shape, `data.reason: "insufficient_scope"` (added at both the outer `tools/call` gate and the inner underlying-ability gate in `ExecuteAbilityAbility::execute()`). `ability_not_public_mcp` added to the `McpErrorMapper` table. Regression-guarded by new tests in `tests/Unit/Handlers/Tools/PermissionErrorRemediationTest.php` and `tests/Unit/Infrastructure/ErrorHandling/McpErrorMapperTest.php`. Closes [#140](https://github.com/Wicked-Evolutions/abilities-mcp-adapter/issues/140).

### Changed

- **`mcp-adapter/discover-abilities` now defaults to a compact, paginated response and always returns a categories histogram (Issue [#139](https://github.com/Wicked-Evolutions/abilities-mcp-adapter/issues/139)).** **Behavior change for no-arg callers:** `discover-abilities { site: "X" }` previously returned the full registry in verbose shape (~411KB / 1,649 abilities on dev2 — over AI-client token caps). It now defaults `compact` to `true` and, when neither `compact` nor `limit` is passed, caps the page at `limit: 100` (reviewer-calibrated on dev2: compact alone is not a size bound — a 305-ability compact category ≈ 25KB, `limit:200` ≈ 17KB once Knowledge-Layer steering is prepended). An explicit `compact` (true or false) suppresses the auto-limit; `compact: false, limit: 0` reproduces the prior verbose unbounded dump verbatim (plus the additive blocks below). Every response now carries a self-documenting `pagination` envelope `{total, filtered_total, returned, offset, next_offset, has_more, compact}` (renamed from the prior limit-only `_pagination` key — the underscore-prefixed key is removed; `total` is the unfiltered `mcp.public`+`type=tool` count, `filtered_total` is the post-filter count) and a `categories` histogram `[{slug, count}]` over the filtered set (pre-pagination, computed in the existing filter loop). Histogram uses each ability's registered category verbatim; the bridge-side shared-prefix bucketing (`surecart-ecommerce` → `surecart`) is a separate follow-up. The 5 `mcp-adapter/*` abilities' meta-shape (#137/#138) is unaffected. Regression-guarded by `tests/Unit/Abilities/DiscoverAbilitiesDefaultsTest.php`. Closes [#139](https://github.com/Wicked-Evolutions/abilities-mcp-adapter/issues/139).
