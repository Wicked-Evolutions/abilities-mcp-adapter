...
use WickedEvolutions\McpAdapter\Domain\Prompts\Contracts\McpPromptBuilderInterface;
use WickedEvolutions\McpAdapter\Domain\Prompts\McpPrompt;
use WickedEvolutions\McpAdapter\Domain\Prompts\RegisterAbilityAsMcpPrompt;
use WickedEvolutions\McpAdapter\Domain\Resources\McpResource;
use WickedEvolutions\McpAdapter\Domain\Resources\RegisterAbilityAsMcpResource;
use WickedEvolutions\McpAdapter\Domain\Tools\McpTool;
use WickedEvolutions\McpAdapter\Domain\Tools\RegisterAbilityAsMcpTool;
...
class McpComponentRegistry {
	 ...
	 * Constructor.
	 *
	 * @param \WickedEvolutions\McpAdapter\Core\McpServer $mcp_server MCP server instance.
	 * @param \WickedEvolutions\McpAdapter\Infrastructure\ErrorHandling\Contracts\McpErrorHandlerInterface $error_handler Error handler instance.
	 * @param \WickedEvolutions\McpAdapter\Infrastructure\Observability\Contracts\McpObservabilityHandlerInterface $observability_handler Observability handler instance.
	...
	public function register_tools( array $tools ): void {
		foreach ( $tools as $tool_item ) {
			...
			// Validate ability name format (2-4 segments: namespace/name).
			if ( ! McpValidator::validate_ability_name( $tool_item ) ) {
				$this->error_handler->log( "Invalid ability name format '{$tool_item}'. Expected 2-4 lowercase segments separated by forward slashes.", array( "RegisterAbilityAsMcpTool::{$tool_item}" ) );
				continue;
			}
			...
			if ( $this->mcp_validation_enabled ) {
				...
				$validation_result = \WickedEvolutions\McpAdapter\Domain\Tools\McpToolValidator::validate_tool_instance( $tool, $context_to_use );
	 ...
	 *
	 * @param \WickedEvolutions\McpAdapter\Domain\Tools\McpTool $tool The tool instance to register.
	 *
	 * @return void
	 */
	...
	public function register_resources( array $abilities ): void {
		foreach ( $abilities as $ability_name ) {
			...
			// Validate ability name format (2-4 segments: namespace/name).
			if ( ! McpValidator::validate_ability_name( $ability_name ) ) {
				$this->error_handler->log( "Invalid ability name format '{$ability_name}'. Expected 2-4 lowercase segments separated by forward slashes.", array( "RegisterAbilityAsMcpResource::{$ability_name}" ) );
				continue;
			}
	...
	public function register_prompts( array $prompts ): void {
		foreach ( $prompts as $prompt_item ) {
			...
			if ( class_exists( $prompt_item ) && in_array( McpPromptBuilderInterface::class, class_implements( $prompt_item ) ?: array(), true ) ) {
				...
				try {
					/** @var \WickedEvolutions\McpAdapter\Domain\Prompts\Contracts\McpPromptBuilderInterface $builder */
				...
				if ( $this->mcp_validation_enabled ) {
					...
					$validation_result = \WickedEvolutions\McpAdapter\Domain\Prompts\McpPromptValidator::validate_prompt_instance( $prompt, $context_to_use );
				...
				// Validate ability name format (2-4 segments: namespace/name).
				if ( ! McpValidator::validate_ability_name( $prompt_item ) ) {
					$this->error_handler->log( "Invalid ability name format '{$prompt_item}'. Expected 2-4 lowercase segments separated by forward slashes.", array( "RegisterAbilityAsMcpPrompt::{$prompt_item}" ) );
					continue;
				}
	 ...
	 *
	 * @return \WickedEvolutions\McpAdapter\Domain\Tools\McpTool[]
	 */
	 ...
	 *
	 * @return \WickedEvolutions\McpAdapter\Domain\Resources\McpResource[]
	 */
	 ...
	 *
	 * @return \WickedEvolutions\McpAdapter\Domain\Prompts\McpPrompt[]
	 */
	 ...
	 *
	 * @return \WickedEvolutions\McpAdapter\Domain\Tools\McpTool|null
	 */
	 ...
	 *
	 * @return \WickedEvolutions\McpAdapter\Domain\Resources\McpResource|null
	 */
	 ...
	 *
	 * @return \WickedEvolutions\McpAdapter\Domain\Prompts\McpPrompt|null
	 */
