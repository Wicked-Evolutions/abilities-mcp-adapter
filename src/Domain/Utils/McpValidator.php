...
class McpValidator {
	...
	/**
	 * Validate a WordPress ability name.
	 *
	 * Ability names use a namespace/name format with 2-4 segments separated by forward slashes.
	 * Each segment must contain only lowercase letters, numbers, and hyphens.
	 * Examples: "content/list", "mcp-adapter/batch-execute", "fluent-crm/contacts/list".
	 *
	 * @param string $name The ability name to validate.
	 *
	 * @return bool True if valid, false otherwise.
	 */
	public static function validate_ability_name( string $name ): bool {
		if ( empty( $name ) || strlen( $name ) > 255 ) {
			return false;
		}

		return (bool) preg_match( '/^[a-z0-9-]+(?:\/[a-z0-9-]+){1,3}$/', $name );
	}
